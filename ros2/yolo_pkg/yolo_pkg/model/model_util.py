...
from model.deformable_conv_v2.deformable_layer import DeformableConv2D
...
class NonMaximumSuppressionBox:
    ...
    def pure_nms(self, pred, merged=False):
        ...
        if cfg.ModelOutput.MINOR_CTGR:
            ...
            mark_dont = tf.cast(categories == self.category_names.index("Road mark dont"), dtype=tf.float32) * \
                        tf.cast(tf.argmax(pred["mark_dont"], axis=-1), dtype=tf.float32)
            mark_do = tf.cast(categories == self.category_names.index("Road mark do"), dtype=tf.float32) * \
                        tf.cast(tf.argmax(pred["mark_do"], axis=-1), dtype=tf.float32)
            minor_ctgr = sign_ctgr + mark_ctgr + mark_dont + mark_do
