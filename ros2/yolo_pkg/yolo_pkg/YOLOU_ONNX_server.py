...
class DetectorServer(Node):
    def __init__(self):
        ...
        self.subscriber = self.create_subscription(Image, 'image_raw', self.listener_callback, qos_profile_sensor_data)
        self.br = CvBridge()
        ...
        self.publisher_ = self.create_publisher(Mystamp, 'box_pub', 1)
        self.box_msg = Mystamp()
    ...
    def listener_callback(self, data):
        ...
        try:
            ...
            print(e)
        decoded_img = cv2.imdecode(frame, 1)
        #cv2.imshow("decode_img", decoded_img)
        #cv2.waitKey(10)
        frame = self.preprocess(decoded_img)
        ...
        if np.size(valid_box["yxhw"]) == 0:
            box_num = 0
        else:
            box_num = valid_box["yxhw"].shape[0]
            
        if np.size(valid_lane["lane_fpoints"]) == 0:
            lane_num = 0
        else:
            lane_num = valid_lane["lane_fpoints"].shape[0]
        self.box_msg.pre_header = data.header
        self.box_msg.box_num = box_num
        self.box_msg.lane_num = lane_num
        self.box_msg.boxes = valid_box["yxhw"].flatten().tolist()
        self.box_msg.boxes_ctgr = valid_box["category"].flatten().tolist()
        self.box_msg.boxes_dist = valid_box["distance"].flatten().tolist()
        self.box_msg.lanes = valid_lane["lane_fpoints"].flatten().tolist()
        self.box_msg.lanes_ctgr = valid_lane["lane_category"].flatten().tolist()
        terminate_t = timeit.default_timer()
        FPS = int(1./(terminate_t - start_t ))
        self.publisher_.publish(self.box_msg)
        self.get_logger().info(f"{FPS}")
        
        #image = self.visual_img(frame[0], self.output_features["inst_box"], self.categories, self.log_keys,self.log_color)
...
def main(args=None):
    ...
    try:
        ...
        detector_server.get_logger().info("keyboard Interrupt (SIGINT)")
    finally:
