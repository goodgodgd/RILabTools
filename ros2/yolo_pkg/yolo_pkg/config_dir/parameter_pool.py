...
class LossComb:
    ...
    UPLUS_MINOR_WEIGHT = {"sign_ctgr": ([2., 1., 1.], "MinorCategoryLoss", "sign_ctgr"),
                          ...
                          "mark_dont": ([2., 1., 1.], "MinorCategoryLoss", "mark_dont"),
                          "mark_do": ([2., 1., 1.], "MinorCategoryLoss", "mark_do"),
                          }
...
class TrainingPlan:
    UPLUS_PLAN = [
        ...
        ("uplus", 20, 0.0001,  LossComb.FULL_COMBINATION, True),
        ("uplus", 20, 0.00001,  LossComb.FULL_COMBINATION, True),
        ("uplus", 20, 0.000001, LossComb.FULL_COMBINATION, True),
    ]
...
class Uplus2Params:
    CATEGORIES_TO_USE = ["보행자", "승용차", "트럭", "버스", "이륜차", "신호등", "자전거", "삼각콘",
                         ...
                         "TS이륜차금지", "TS우회전금지", "TS좌회전금지", "TS유턴금지", "TS주정차금지", "TS유턴",
    ...
    CATEGORY_REMAP = {"보행자": "Pedestrian",  "승용차": "Car", "트럭": "Truck", "버스": "Bus",
                      ...
                      "TS유턴금지": "TS_NO_TURN", "TS주정차금지": "TS_NO_STOP",
...
class TfrParams:
    # # [300 0 0 0] crop 픽셀 50% Road makr 20
    MIN_PIX = {
        'train': {"Bgd": 0, "Pedestrian": 68, "Car": 87, "Truck": 98, "Bus": 150, "Motorcycle": 38,
                  "Traffic light": 41, "Traffic sign": 26, "Road mark": 20, "Road mark dont": 20, "Road mark do": 20,
                  "Bicycle": 38, "Cone": 34, "Lane_stick": 30, "Bump": 75, "Pothole": 0
                  },
        'val': {"Bgd": 0, "Pedestrian": 76, "Car": 98, "Truck": 110, "Bus": 168, "Motorcycle": 42,
                "Traffic light": 46, "Traffic sign": 29, "Road mark": 20, "Road mark dont": 20, "Road mark do": 20,
                "Bicycle": 42, "Cone": 38, "Lane_stick": 34, "Bump": 85, "Pothole": 0
                },
    }
    ...
    CATEGORY_NAMES = {"major": ["Bgd", "Pedestrian", "Car", "Truck", "Bus", "Motorcycle", "Traffic light",
                                "Traffic sign", "Road mark", "Road mark dont", "Road mark do", "Bicycle", "Cone",
                                "Lane_stick", "Bump", "Pothole"],
                      "sign": ["TS_NO_TW", "TS_NO_RIGHT", "TS_NO_LEFT", "TS_NO_TURN", "TS_NO_STOP",
                               "TS_U_TURN", "TS_CHILDREN", "TS_CROSSWK", "TS_GO_LEFT",
                               "TS_SPEED_LIMIT"],
                      "mark": ["RM_U_TURN", "RM_ANN_CWK", "RM_CROSSWK", "RM_SPEED_LIMIT"],
                      "mark_dont": ["RM_NO_RIGHT", "RM_NO_LEFT", "RM_NO_STR"],
                      "mark_do": ["RM_GO_RIGHT", "RM_GO_LEFT", "RM_GO_STR"],
...
class TrainParams:
    ...
    def get_pred_composition(cls, minor_ctgr, speed_limit, iou_aware, categorized=False):
        ...
        if minor_ctgr:
            ...
            cls_composition["mark_dont"] = len(TfrParams.CATEGORY_NAMES["mark_dont"])
            cls_composition["mark_do"] = len(TfrParams.CATEGORY_NAMES["mark_do"])
        if speed_limit:
