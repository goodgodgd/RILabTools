...
class ExhaustiveBoxLog:
    def __init__(self, loss_names):
        ...
        if cfg.ModelOutput.MINOR_CTGR:
            ...
            self.mark_dont_summary = dict()
            self.mark_do_summary = dict()
    ...
    def __call__(self, step, grtr, pred, loss, total_loss):
        ...
        metric, sign_metric, mark_metric, mark_dont_metric, mark_do_metric = self.box_category_match(grtr, pred["inst_box"], range(1, self.num_categs), step)
        ...
        for category in categories:
            ...
            if cfg.ModelOutput.MINOR_CTGR:
                if category == cfg.Dataloader.CATEGORY_NAMES["major"].index("Traffic sign"):
                    ...
                    minor_metric = count_true_positives(grtr_match_box, pred_match_box, grtr["inst_dc"],
                                                        self.num_categs,
    ...
    def compute_sum_summary(self):
        ...
        if cfg.ModelOutput.MINOR_CTGR:
            ...
            sign_grtr = np.sum(np.stack(sign_data["minor_grtr"].tolist(), axis=0), axis=0)
            sign_pred = np.sum(np.stack(sign_data["minor_pred"].tolist(), axis=0), axis=0)
            ...
            mark_grtr = np.sum(np.stack(mark_data["minor_grtr"].tolist(), axis=0), axis=0)
            mark_pred = np.sum(np.stack(mark_data["minor_pred"].tolist(), axis=0), axis=0)
            ...
            mark_dont_grtr = np.sum(np.stack(mark_dont_data["minor_grtr"].tolist(), axis=0), axis=0)
            mark_dont_pred = np.sum(np.stack(mark_dont_data["minor_pred"].tolist(), axis=0), axis=0)
            mark_dont_summary = pd.DataFrame({"recall": mark_dont_trpo / (mark_dont_grtr + 1e-5),
                                         "precision": mark_dont_trpo / (mark_dont_pred + 1e-5),
                                         "grtr": mark_dont_grtr, "pred": mark_dont_pred,
                                         "trpo": mark_dont_trpo})
            mark_do_data = self.mark_do_metric_data
            mark_do_trpo = np.sum(np.stack(mark_do_data["minor_trpo"].tolist(), axis=0), axis=0)
            mark_do_grtr = np.sum(np.stack(mark_do_data["minor_grtr"].tolist(), axis=0), axis=0)
            mark_do_pred = np.sum(np.stack(mark_do_data["minor_pred"].tolist(), axis=0), axis=0)
            mark_do_summary = pd.DataFrame({"recall": mark_do_trpo / (mark_do_grtr + 1e-5),
                                         "precision": mark_do_trpo / (mark_do_pred + 1e-5),
                                         "grtr": mark_do_grtr, "pred": mark_do_pred,
                                         "trpo": mark_do_trpo})
