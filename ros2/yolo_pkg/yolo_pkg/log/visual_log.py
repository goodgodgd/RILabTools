...
class VisualLog:
    def __init__(self, ckpt_path, epoch):
        ...
        self.mark_dont_ctgr = {i: ctgr_name for i, ctgr_name in enumerate(cfg.Dataloader.CATEGORY_NAMES["mark_dont"])}
        self.mark_do_ctgr = {i: ctgr_name for i, ctgr_name in enumerate(cfg.Dataloader.CATEGORY_NAMES["mark_do"])}
    ...
    def draw_boxes(self, image, bboxes, frame_idx, log_keys, color):
        ...
        for i in range(box_yxhw.shape[0]):
            ...
            if cfg.ModelOutput.MINOR_CTGR:
                ...
                elif annotation == "Road mark dont":
                    annotation = f"{self.mark_dont_ctgr[minor_ctgr[i]]}"
                elif annotation == "Road mark do":
                    annotation = f"{self.mark_do_ctgr[minor_ctgr[i]]}"
