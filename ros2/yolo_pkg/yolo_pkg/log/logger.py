...
from log.dist_exhaustive_log import ExhaustiveDistLog
...
class Logger:
    def __init__(self, visual_log, exhaustive_log, loss_names, ckpt_path, epoch, is_train, val_only):
        ...
        self.exhaustive_dist_logger = ExhaustiveDistLog() if exhaustive_log else None

        self.visual_logger = VisualLog(ckpt_path, epoch) if visual_log else None
        self.history_filename = op.join(ckpt_path, "history.csv")
        self.history_time_filename = op.join(ckpt_path, "history_time.csv")
    ...
    def log_batch_result(self, step, grtr, pred, total_loss, loss_by_type):
        ...
        if self.exhaustive_log:
            ...
            self.exhaustive_dist_logger(step, grtr, pred, loss_by_type, total_loss)
            if cfg.ModelOutput.LANE_DET:
    ...
    def finalize(self, start):
        ...
        if self.exhaustive_log:
            ...
            self.save_exhaustive_log(start, self.exhaustive_dist_logger, "dist")
            if cfg.ModelOutput.LANE_DET:
    ...
    def save_exhaustive_log(self, start, exhaustive_logger, name):
        ...
        if self.val_only:
            ...
            if cfg.ModelOutput.MINOR_CTGR and (name != "lane"):
                sign_summary, mark_summary, mark_dont_summary, makr_do_summary = exhaustive_logger.get_minor_summary()
                sign_filename = self.exhaust_path + f"/sign_{name}_val.csv"
                mark_filename = self.exhaust_path + f"/mark_{name}_val.csv"
                mark_do_filename = self.exhaust_path + f"/mark_dont_{name}_val.csv"
                mark_dont_filename = self.exhaust_path + f"/mark_do_{name}_val.csv"
                ...
                mark_dont_summary.to_csv(mark_do_filename, encoding='utf-8', index=False, float_format='%.4f')
                makr_do_summary.to_csv(mark_dont_filename, encoding='utf-8', index=False, float_format='%.4f')
        else:
    ...
    def save_val_log(self):
        ...
        # history_time_summary = pd.DataFrame()
        # for key, val in logger_summary_time.items():
        #     history_time_summary = history_time_summary.append({key,val})
        #
